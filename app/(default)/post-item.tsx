...
export default function PostItem({ ...props }) {
  return (
    <div className={`nth-[-n+12]:-order-1 group ${!props.sticky && 'border-b border-gray-200'}`}>
          ...
          <div className="grow lg:flex items-center justify-between space-y-5 lg:space-x-2 lg:space-y-0">
            ...
            <div className="min-w-[120px] flex items-center lg:justify-end space-x-3 lg:space-x-0">
              <div className="lg:hidden lg:group-hover:block">
                <Link className="btn-sm py-1.5 px-3 text-white bg-indigo-500 hover:bg-indigo-600 group shadow-xs" href={`/posts/${props.id}`}>
                  Apply Now{' '}
              ...
              <div className="lg:group-hover:hidden text-sm italic text-gray-500">{props.date}</div>
            </div>
          </div>
        </div>
      </div>
